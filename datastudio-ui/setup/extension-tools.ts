...
/// <reference path="./css-url-parser.d.ts" />
...
import { IUserFriendlyViewsContainerDescriptor } from "@codingame/monaco-vscode-view-common-service-override/vscode/vs/workbench/api/browser/viewsExtensionPoint";
import {
  parse,
  ParseError,
} from "@codingame/monaco-vscode-api/vscode/vs/base/common/json";
...
async function extractThemeResources(
  ...
  const themeDocument = parseJson<IconThemeDocument>(
    theme.path,
    themeContent.toString("utf8"),
  );
  ...
  if (themeDocument.iconDefinitions != null) {
    for (const iconDefinition of Object.values(themeDocument.iconDefinitions)) {
      if (iconDefinition.iconPath != null) {
        paths.push(
          path.join(path.dirname(theme.path), iconDefinition.iconPath),
        );
      }
    }
  }
...
function extractJsonValidationResources(
...
function extractIconResources(
  icons: NonNullable<IExtensionContributions["icons"]>,
): string[] {
...
async function extractResourcesFromExtensionManifestContribute(
  ...
  const resources: string[] = [];
  if (contribute.icons != null)
    resources.push(...extractIconResources(contribute.icons));
  if (contribute.commands != null)
    resources.push(...extractCommandResources(contribute.commands));
  ...
  if (contribute.themes != null)
    resources.push(
      ...(
        await Promise.all(
          contribute.themes.map(theme => extractThemeResources(theme, fs, cwd)),
        )
      ).flat(),
    );
  if (contribute.iconThemes != null)
    resources.push(
      ...(
        await Promise.all(
          contribute.iconThemes.map(theme =>
            extractThemeResources(theme, fs, cwd),
          ),
        )
      ).flat(),
    );
  if (contribute.productIconThemes != null)
    resources.push(
      ...(
        await Promise.all(
          contribute.productIconThemes.map(theme =>
            extractThemeResources(theme, fs, cwd),
          ),
        )
      ).flat(),
  ...
  if (contribute.jsonValidation != null)
    resources.push(
      ...contribute.jsonValidation.flatMap(extractJsonValidationResources),
    );
  if (contribute.viewsContainers != null)
    resources.push(
      ...extractViewsContainerResources(contribute.viewsContainers),
    );
  ...
  return resources.filter(
    (resource, index, list) =>
      !resource.startsWith("$(") &&
      !list.slice(0, index).some(o => o === resource),
  );
}
...
export async function getExtensionResources(
  ...
  if (
    ...
    return (await extractResourcesFromExtensionManifest(manifest, fs, cwd)).map(
      p => toResource(p, fs, cwd),
    );
  }
}
...
async function extractResourcesFromExtensionManifest(
  ...
  if (manifest.contributes != null) {
    resources.push(
      ...(await extractResourcesFromExtensionManifestContribute(
        <IExtensionContributions & { [customKey: string]: unknown }>(
  ...
  const manifestFiles = await glob(
    "{package.nls.json,package.nls.*.json,package.json}",
    {
      fs: <FileSystemAdapter>fs,
      cwd,
      onlyFiles: true,
